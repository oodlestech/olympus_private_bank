    ...
    <div class="row align-items-center">
            ...
            <div class="p-12 mw-md mx-auto bg-dark" style="margin-bottom:20px; padding: 20px; background-color: #598FFA !important;">
              ...
              <ul class="list-unstyled mb-10" style="color:#eee">
                <li class="text-#eee lh-lg d-flex mb-2">
                  ...
                  <span>{{ __('Period of Staking') }} {{$inv->period}} {{__('Days')}}</span>
                </li>
                <li class="text-#eee lh-lg d-flex mb-2">
                  <i class="fa fa-cube tx-primary mg-r-8" style="display: inline-block; margin-right:10px"></i>
                  <span>{{ __('Min Staking') }} {{$settings->currency}} {{$inv->min}}</span>
                </li>
                <li class="text-#eee lh-lg d-flex mb-2">
                  <i class="fa fa-cube tx-primary mg-r-8" style="display: inline-block; margin-right:10px"></i>
                  <span>{{ __('Max Staking') }} {{$settings->currency}} {{$inv->max}}</span>
                </li>
                <li class="text-#eee lh-lg d-flex mb-2">
                  <i class="fa fa-cube tx-primary mg-r-8" style="display: inline-block; margin-right:10px"></i>
                  <span>{{ __('APR') }} {{$inv->daily_interest*$inv->period*100}}%</span>
                </li>
                <li class="text-#eee lh-lg d-flex">
                  <i class="fa fa-cube tx-primary mg-r-8" style="display: inline-block; margin-right:10px"></i>
                  <span>{{ __('Unstaking Interval') }} {{$inv->days_interval}} Days</span>
                </li>
              </ul>
              <div class="" style="color: #fff" align="center">
                  <p>{{ __('Funds accessible after staking elapses.') }}</p>
              </div>
      ...
      @else
        <div class="alert alert-warning">
            <a href="/{{$user->username}}/profile#userdet">{{ __('Please, click here to update your profile before you can stake.') }}</a>
        </div>
      @endif
