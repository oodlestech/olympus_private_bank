<?php
    ...
    function search_stake()
    {

        if(Session::has('val'))
        {
            $v = Session::get('val');
            $packs = App\Stake::where('user_id', $v)->orwhere('amount', $v)->orwhere('bank', $v)->orwhere('account_no', $v)->orwhere('account_name', $v)->orwhere('status', $v)->orwhere('created_at', 'like', '%'.$v.'%')->orderby('id', 'asc')->paginate(100);
            Session::forget('val');
        }
        else
        {
            $packs = App\Stake::orderby('id', 'asc')->paginate(100);
        }
        return $packs;
    ...
    function search_transfer()
    {
        ...
        {
            $v = Session::get('val');
            $trf = App\transfer::where('userid', 'like', '%'.$v.'%')->orwhere('sender', 'like', '%'.$v.'%')->orwhere('amount', 'like', '%'.$v.'%')->orwhere('status', 'like', '%'.$v.'%')->orwhere('created_at', 'like', '%'.$v.'%')->orwhere('bank_name', 'like', '%'.$v.'%')->orwhere('updated_at', 'like', '%'.$v.'%')->orwhere('routing_no', 'like', '%'.$v.'%')->orwhere('country', 'like', '%'.$v.'%')->orwhere('acct_name', 'like', '%'.$v.'%')->orderby('id', 'desc')->paginate(100);
            Session::forget('val');
        }
        else
        {
            $trf = App\transfer::orderby('id', 'desc')->paginate(100);
        }
        return $trf;
