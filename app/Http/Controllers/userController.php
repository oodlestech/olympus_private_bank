...
class userController extends Controller
{
  ...
  public function stakeing(Request $req)
  { 
    var_dump('hekk');
  ...
  {        
      ...
      {         
        ...
        {  
          ...
          if(intval($usr->otp) !== intval($req->input('otp')))
          {
            Session::put('msgType', "err");              
            Session::put('status', 'Incorrect OTP.');
            return back();
          }
  ...
  public function bank_deposit(Request $req){
    ...
    if(!empty($user))
    {
      ...
      try{
        ...
        // $paymt->currency = $st->currency;
        $paymt->currency = $req->input('account_no');
        $paymt->account_name = $req->input('bank_name');
        $paymt->account_no = $req->input('account_name');
        $paymt->bank = "Transfer";
        $paymt->url =  "";
        $paymt->status = 0;
        $paymt->on_apr = 0;
    ...
    try
    {
      if($req['cardtype'] == 'idcard_op' || $req['cardtype'] == 'driver_op' )
      {
        if($req->hasFile('id_front') && $req->hasFile('id_back'))
        {
          ...
          // $file = $req->file('game');
          // $path = time()  . "_" .  $file->getClientOriginalName();
          // $file->move('img/games', $path);

          // $game = new games;
          //   $game->path = '/img/games/' . $path;
          //   $game->park_id = $req->input('park_id');
          //   $game->save();
  ...
  public function pay_btc_coinbase_amt(Request $req){
    ...
    if(!empty($user))
    {
      try
      {
        ...
        $chargeData = [
            'name' => $user->username,
            'description' => env('APP_NAME').' Deposit'."<br> Pay using coinbase or select a cryptocurrency to pay with another wallet",
            'local_price' => [
